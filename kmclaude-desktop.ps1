#!/usr/bin/env pwsh
# kmclaude-desktop.ps1 — configure Claude Desktop to use Moonshot AI Kimi as inference backend (Windows port).
#
# Edits %APPDATA%\Claude-3p\configLibrary\{_meta,<uuid>}.json and restarts Claude Desktop.
# This is the Windows companion to the macOS bash version `kmclaude-desktop` —
...
#
# Usage:
#   pwsh ./kmclaude-desktop.ps1                          # auto-detect and configure
#   pwsh ./kmclaude-desktop.ps1 -ClaudeExePath <path>    # specify custom Claude.exe
#   pwsh ./kmclaude-desktop.ps1 -Update                  # git pull latest from the repo
#   pwsh ./kmclaude-desktop.ps1 -h                       # help
#
# Reads KIMI_API_KEY from env (else prompts). The base URL defaults to
# https://api.moonshot.cn/anthropic; override with $env:KIMI_BASE_URL.
#
# Requires: PowerShell 5.1+ (Windows 10+ ships this), Claude Desktop installed,
# Developer Mode enabled in Claude Desktop, Moonshot Kimi API key.
...
if ($Update) {
    ...
    Write-Host "kmclaude-desktop: pulling latest from $repo ..."
    git -C $repo pull
    if ($LASTEXITCODE -eq 0) {
        Write-Host 'kmclaude-desktop: updated.'
    } else {
        Write-Error 'kmclaude-desktop: git pull failed. Check network or resolve conflicts manually.'
        exit 1
    }
    exit 0
}
...
function Test-Preflight {
    ...
    if (-not $isWin) {
        Write-Error 'kmclaude-desktop.ps1: Windows only. Use ./kmclaude-desktop on macOS.'
        exit 1
    }

    if ($script:ClaudeExePath) {
        if (-not (Test-Path $script:ClaudeExePath)) {
            Write-Error "kmclaude-desktop.ps1: Claude.exe not found at '$script:ClaudeExePath'"
            exit 1
        }
        ...
        if (-not $found) {
            Write-Error @"
kmclaude-desktop.ps1: Claude Desktop not found.
...
"@
    ...
    if ($needsEnable) {
        Write-Host 'kmclaude-desktop.ps1: Enabling Developer Mode for Claude Desktop...'
        ...
        Write-Host 'kmclaude-desktop.ps1: Developer Mode enabled.'
    }
}
...
function Resolve-ApiKey {
    ...
    $secure = Read-Host 'KIMI_API_KEY not set. Paste your Moonshot Kimi API Key' -AsSecureString
    if (-not $secure -or $secure.Length -eq 0) {
        Write-Error 'kmclaude-desktop.ps1: no Kimi API Key provided. Aborting.'
        exit 1
    }
...
Confirm-OrAbort -Action "configure Claude Desktop to use Moonshot Kimi ($script:BaseUrl) and restart it."
...
Done. Claude Desktop is restarting with Moonshot Kimi as the inference backend.
...
Re-run kmclaude-desktop.ps1 any time to refresh the gateway config.
"@ | Write-Host
